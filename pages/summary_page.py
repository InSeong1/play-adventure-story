...
def summary_page():
    ...
    # 간단한 인쇄 안내
    st.markdown("### 💡 PDF 저장 팁")
    
    st.markdown("**🖥️ PC에서:**")
    st.markdown("**위의 버튼을 클릭하거나** **Ctrl + P** (Windows/Linux) 또는 **Cmd + P** (Mac)을 눌러주세요!")
    st.markdown("**인쇄 창에서:** 대상을 'PDF로 저장'으로 변경하고, '배경 그래픽' 옵션을 체크하면 색상과 이미지가 포함된 완벽한 PDF를 만들 수 있습니다.")
    
    st.markdown("**📱 태블릿에서:**")
    st.markdown("**iPad:** 화면을 길게 누르고 '공유' → 'PDF로 저장' 또는 Safari에서 **Cmd + P** → 'PDF로 저장'")
    st.markdown("**Android 태블릿:** Chrome에서 ⋮ 메뉴 → '공유' → '인쇄' → 'PDF로 저장' 또는 화면 캡처 후 'PDF로 변환' 앱 사용")
    st.markdown("**공통:** 브라우저 설정에서 '데스크톱 모드'로 전환하면 PC와 동일한 방법으로 PDF 저장이 가능합니다!")
    
    st.markdown("*💡 왼쪽 메뉴 바를 닫고 저장을 시도해서 pdf 파일로 증서를 얻어볼까요?*")
